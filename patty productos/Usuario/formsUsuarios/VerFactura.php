...
<form action="verfactura.php" method="post" id="ven" name="form" class="navbar-form">
    <fieldset>
...
switch ($opcion) {
  ...
  $sql="SELECT f.id as Numero,f.numerofactura as '#Factura',f.carnetusuario as 'Carnet Vendedor'";
  $sql.=",f.fechaventa as 'Fecha de Venta',u.nombre as 'Nombre Vendedor'";
  $sql.=",u.apellido as 'Apellido Vendedor' FROM facturaventa f INNER JOIN usuario u ON f.carnetusuario=u.carnet WHERE f.carnetusuario=u.carnet AND u.usuario='$user' AND u.contrasena='$pws' and numerofactura='$nombreSelecion' ORDER BY f.id asc ";
  $datos=consultaD($con,$sql);
  print imprimeSelect($datos,"facturaventa/buscardetalle/Detalle","table table-bordered table-striped",2,TRUE);
print"<a id='lin' href='verfactura.php'>Ver Listado de Facturas</a><legend id='linea'></legend>";
    break;
