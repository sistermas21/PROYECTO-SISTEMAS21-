...
<!--menu-->
    ...
    <form action="FormularioConsultarComprapormes.php" method="post" id="compra" name="form" class="navbar-form">
    <fieldset>
...
if (!isset($_REQUEST['buscar'])) {
  ...
  print imprimetablaF1($datos,"./buscardetalle/Detalled","table able-bordred tabl-striped",2,TRUE);
...
switch ($opcion) {
  case 0:
    ...
    break;
    case 1:
  $sql="SELECT F.id as Numero,F.numerofactura as'#Factura',P.nombre as Proveedor,F.fechacompra as 'Fecha de Compra' FROM facturacompra F INNER JOIN proveedor P on P.id=F.codigoproveedor WHERE numerofactura='$nombreSelecion' ORDER BY F.id asc";
  $datos=consultaD($con,$sql);
  print imprimetablaF($datos,"./buscardetalle/Detalled","table table-bordred tbl-striped",2,TRUE);
print"<a id='lin' href='FormularioConsultarComprapormes.php'>Ver Listado de Facturas</a><legend id='linea'></legend>";
    break;
    case 2:
   $sql="SELECT F.id as Numero,F.numerofactura as '#Factura',P.nombre as Proveedor,F.fechacompra as 'Fecha de Compra' FROM facturacompra F INNER JOIN proveedor P on P.id=F.codigoproveedor where P.nombre='$nombreSelecion' ORDER BY F.id asc";
  $datos=consultaD($con,$sql);
    print imprimetablaF($datos,"./buscardetalle/Detalled","table table-bodeed tble-strped",2,TRUE);
    print"<a id='lin' href='FormularioConsultarComprapormes.php'>Ver Listado de Facturas</a><legend id='linea'></legend>";
   break;
